...
router.post('/submit-upi-payment', async (req, res) => {
  try {
    const {
      ...
      pricePerTicket: bodyPricePerTicket,
      ticketPrice: bodyTicketPrice,
      quantity,
      upiTransactionId,
    } = req.body;

    const pricePerTicketVal = bodyPricePerTicket || bodyTicketPrice;

    // 1. Basic validation
    if (!name || !email || !phone || (!bodyTicketType && pricePerTicketVal === undefined) || !quantity || !upiTransactionId) {
      return res.status(400).json({ success: false, message: 'Missing required order or payment details.' });
    }
    ...
    // 3. Pricing and Ticket Type calculations
    let pricePerTicket;
    if (pricePerTicketVal !== undefined && pricePerTicketVal !== null) {
      pricePerTicket = parseInt(pricePerTicketVal, 10);
    } else {
      pricePerTicket = bodyTicketType === 'VIP'
        ? parseInt(process.env.EVENT_PRICE_VIP || '1000', 10)
        : parseInt(process.env.EVENT_PRICE_GENERAL || '500', 10);
    }

    const ticketType = pricePerTicket <= 500 ? 'General' : 'VIP';
    const totalAmount = pricePerTicket * qty;
